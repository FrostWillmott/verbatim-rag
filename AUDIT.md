...
|---|---:|---:|---:|---:|---:|---:|
| Dependency hygiene | 18/100 | 10 | 6 | 3 | 1 | 0 |
| Security | 25/100 | 5 | 5 | 0 | 0 | 0 |
| CI/CD | 32/100 | 6 | 2 | 1 | 2 | 1 |
...
| DEP-4 | `datasets 3.4.1` — CVE-2026-66007 | medium | `todo` | An earlier note here blamed `uv pip compile` for preserving pins. That was wrong and worth correcting rather than deleting: `datasets==3.4.1` is an exact pin in `pyproject.toml:31`, so the procedure is the one that closed DEP-2 — edit the manifest, `make lock`, regenerate the container lock, re-run `pip-audit`. The honest reason it is still open is that it was not reached, not that the tooling resisted. |
...
| CD-002 | Executable specs strong for core, weak for full RAG / API / frontend | medium | `todo` | Largely addressed by TST-1 and TST-2, which pin the RAG path and the API surface. Left open because the third surface the finding names — the frontend — still has no executable specification at all, and that is TST-4. |
