...
Tests 97 → 190, plus 7 UI tests where there was no framework. Project coverage
19% → 41.18%, with a CI floor that fails below 35. `npm audit` 13 findings, 10
...
| CFG-4 | Runtime decisions hidden as code constants | low | `question` | The LLM model, its endpoint, the embedding model and the Milvus collection name `"acl"` are literals in `api/dependencies.py:35,36,40,47`. **The question:** which of these are deliberate constants of a demo and which are settings that were never wired? `"acl"` in particular reads as a binding to one specific corpus. Promoting them blindly would invent a configuration contract; leaving them silent is the finding. Only the maintainer knows which. **Two of the four moved since, and the question did not.** `BEY-12` produced a case the question cannot wait for: the model literal is not reachable from every Groq account, and when it is not, the stack answers every question with "no relevant information found". So the model and the endpoint are `LLM_MODEL` and `LLM_API_BASE` in `APIConfig` — the settings mechanism this API already had — with the previous literals as defaults, byte for byte, and a contract test pinning them there. **What that does and does not settle:** it removes the need to edit source to reach a working model, which is a demo defect; it takes no position on which model or endpoint is right, which is still the maintainer's. The embedding model and `"acl"` are deliberately left alone, and the reason first written here — that a setting would change where the API reads — does not survive a literal reading: a setting whose default is the current literal changes nothing, exactly as `LLM_MODEL` did not. The real argument is the one `BEY-9` measured. Both embedding models in play emit 384 dimensions, so a wrong value does not fail: it loads vectors from one model into an index built by another and returns confident nonsense, and a changed collection name silently reads an index nobody wrote. A knob whose misuse is undetectable is worse than an edit to source, which at least leaves a diff. Exposing them needs a guard that refuses a mismatch between model and collection, and that guard needs the maintainer's answer about which pairing is authoritative — which is this row's question. |
